    ...
    <div>
        <div class="sixteen wide tablet twelve wide computer column">
            <div class="ui red message" v-if="!isPostable">
                ...
                <p>Please increase your post create limit by visting <a :href="'/' + businessDomain + '/profile'" class="link">account page</a>.</p>
            </div>

            <div class="ui green message" v-if="postLimit > 0 && role !== 'redeeming'">
                <div class="header">You have {{postLimit}} posts left.</div>
            </div>

            <div v-if="(role !== 'redeeming') && isPostable">
                <div class="ui header" v-if="!isEditing">Create a Request Post</div>
                    ...
                    <div class="content">
                        <div class="ui form create-form">
                            ...
                            <div class="field" v-if="previousImage">
                                ...
                                <img class="ui image" v-bind:src="previousImage">
                            </div>
                            <div class="field">
                                <label>What they will get?</label>
                                <input type="text" placeholder="Start with get... (required)" maxlength="120" v-model="shortDescription" @keyup="checkShortDescriptionLimit($event)" @keydown="checkShortDescriptionLimit($event)">
                                <span class="text-fade">{{120 - shortDescLimit}} Characters left</span>
                            </div>
                            <div class="field">
                                <label>Add Rules of Request</label>
                                <textarea placeholder="Add Rules of Request" maxlength="400" v-model="content" @keyup="checkLongDescriptionLimit($event)" @keydown="checkLongDescriptionLimit($event)"></textarea>
                                <span class="text-fade">{{400 - longDescLimit}} Characters left</span>
                            </div>
                            <div class="two fields">
                                <div class="field middle aligned">
                                    <label>Coupon</label>
                                    <input class="coupon-input popup-element" minlength="4" v-model="coupon" type="text" placeholder="Enter 4 digit coupon code" maxlength="4" data-content="If you dont set any coupon value a 4 digit value will be set">
                                </div>
<!--                                <div class="field">-->
...
<!--                                </div>-->
                ...
                <button class="ui btn-primary button action-button" @click="updatePost()" v-if="!isEditing">Create</button>
                <button class="ui btn-primary button action-button" @click="updatePost()" v-if="isEditing">Update</button>
            </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                dropzoneOptions: {
                    ...
                    acceptedFiles: 'image/*',
                    params: {
                        post_id: (this.editablePost && Object.keys(this.editablePost).length > 0 ? this.editablePost.id : '')
                    },
                    headers: {
        ...
        mounted() {
            ...
            if(this.editablePost && Object.keys(this.editablePost).length > 0){
                ...
                this.expiresIn = this.editablePost.expires_in ? this.editablePost.expires_in : '';
        ...
        methods:{
            isValidData() {
                if(
                    ...
                    this.shortDescription.length > 4
                ){
                    return true;
                }
            ...
            updatePost(choosePayment = false) {
                ...
                NProgress.start();
                axios.put('/api/posts/'+ this.postId, formData).then(response => {
                    NProgress.done();
